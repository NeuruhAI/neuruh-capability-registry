...
class ArgField:
    ...
    def from_mapping(cls, raw: Mapping[str, Any]) -> "ArgField":
        ...
        if field_type not in {"string", "number", "boolean", "string[]"}:
            raise CapabilityError("E_SCHEMA", f"unsupported argument type: {field_type}")
        return cls(
            type=field_type,
...
class Capability:
    ...
    def from_mapping(cls, raw: Mapping[str, Any]) -> "Capability":
        ...
        return cls(
            ...
            allowed_target_types=tuple(str(x) for x in raw.get("allowed_target_types", ())),
        )
...
class CapabilityRegistry:
    ...
    def __init__(self, capabilities: Sequence[Capability]):
        ...
        for cap in capabilities:
            if cap.operation in by_operation:
                raise CapabilityError("E_DUPLICATE", f"duplicate capability: {cap.operation}")
    ...
    def resolve(self, operation: str) -> Capability:
        try:
            ...
            raise CapabilityError("E_CAPABILITY_UNKNOWN", f"unknown capability: {operation}") from exc
    ...
    def validate_args(self, operation: str, args: Mapping[str, Any]) -> None:
        ...
        if unknown:
            raise CapabilityError("E_ARGUMENTS", f"unknown argument(s): {', '.join(unknown)}")

        for name, spec in cap.arg_schema.items():
            if name not in args:
                if spec.required:
                    raise CapabilityError("E_ARGUMENTS", f"missing required argument: {name}")
                continue
            value = args[name]
    ...
    def _validate_value(name: str, value: Any, spec: ArgField) -> None:
        if spec.type == "string":
            ...
            if not isinstance(value, list) or not all(isinstance(x, str) for x in value):
                raise CapabilityError("E_ARGUMENTS", f"{name} must be an array of strings")
            if spec.max_items is not None and len(value) > spec.max_items:
                raise CapabilityError("E_ARGUMENTS", f"{name} exceeds max_items")
            if spec.max_length is not None and any(len(x) > spec.max_length for x in value):
                raise CapabilityError("E_ARGUMENTS", f"{name} contains an item exceeding max_length")
